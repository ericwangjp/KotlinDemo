 ...
 * 调用：
 *      doubleReference(1,2,::sumTotal)
 * 在 kotlin 中有一个约定，如果最后一个参数是函数，可以省略括号
 */
fun sumTotal(num1: Int, num2: Int) = num1 + num2

fun doubleReference(num: Int, num1: Int, method: (Int, Int) -> Int) {
    println(num + method(num, num1))
    val ui = MyUI()
    println(nonPara(ui::testm))
}

fun nonPara(m: () -> Unit) {
    m()
    println("non params method")
    val mlist = listOf("jack", "lucky", "john", "libai")
    for ((index, value) in mlist.withIndex()) {
        println("index is $index,and value is $value")
    }
}

tailrec fun <T> testFun(t: T): Int {
    return if (false) t as Int else {
        println("执行中...")
        testFun(456)
    }
}

val h1 = { x: Int, y: Int -> x + y }
val h2: (Int, Int) -> Int = { x, y -> x + y }
val h3: (Int) -> Int = { it + 1 }
val h4: (Int) -> Int = {
    println("it-> $it")
    var i = it + 1
